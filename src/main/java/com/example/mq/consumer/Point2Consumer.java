...
@RabbitListener(bindings = @QueueBinding(
        value = @Queue(value = "#{T(com.example.mq.constant.PointMqEnum).SIMPLE2.getQueue()}", durable = "true")
        , exchange = @Exchange("#{T(com.example.mq.constant.PointMqEnum).SIMPLE2.getExchange()}")
        , key = "#{T(com.example.mq.constant.PointMqEnum).SIMPLE2.getRoutingKey()}"))
// 这种注解方式，当队列不存在时，会报错
// @RabbitListener(queues = "#{T(com.example.mq.constant.PointMqEnum).SIMPLE.getQueue()}")
public class Point2Consumer implements Consumer {
    ...
    public void recv(String message) {
        System.out.printf("消费消息：队列[%s]——消息内容“%s”%n", PointMqEnum.SIMPLE2.getQueue(), message);
    }
